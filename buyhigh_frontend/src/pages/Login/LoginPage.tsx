...
const LoginPage: React.FC = () => {
  ...
  const { login, loginWithGoogle, loginAsGuest, loading } = useAuth();
  ...
  const handleGuestLogin = async () => {
    setError('');
    frontendLogger.info('Guest login attempt');
    try {
      const success = await loginAsGuest();
      if (success) {
        frontendLogger.info('Guest login successful, redirecting to dashboard');
        navigate('/dashboard', { replace: true });
      } else {
        setError('Guest login failed. Please try again.');
        frontendLogger.warn('Guest login failed: Firebase or backend issue');
      }
    } catch (err) {
      setError('An error occurred during guest login. Please try again.');
      frontendLogger.error('Guest login error', { error: err instanceof Error ? err.message : String(err) });
            ...
            <div className="mt-6">
              <div>
                <button
                  ...
                  disabled={loading}
                  className="group relative w-full flex justify-center py-3 px-4 border border-gray-300 dark:border-gray-600 text-sm font-medium rounded-md text-gray-700 dark:text-gray-300 bg-white dark:bg-gray-800/40 hover:bg-gray-50 dark:hover:bg-gray-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 disabled:opacity-50 transition-colors duration-150 ease-in-out"
                >
                  {loading ? (
